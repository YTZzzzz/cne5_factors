...
def get_multiperiod_stock_returns(stock_list, latest_trading_date, parameters):
    ...
    daily_stock_return = rqdatac.get_price_change_rate(stock_list,start_dates[0],end_dates[-1]) * 100

    multiperiod_daily_stock_return = {}

    for i in range(1, parameters.get('Newey_West_Auto_Correlation_Lags') + 1):

        multiperiod_daily_stock_return['lag_' + str(i)] = daily_stock_return[-(parameters.get('factor_return_length') + i): -i]

    # 返回当期的因子收益序列，以及滞后N期的因子收益序列

    return daily_stock_return[-parameters.get('factor_return_length'):], multiperiod_daily_stock_return
...
def Newey_West_adjustment(daily_stock_return, multiperiod_daily_stock_return, parameters):

    # 计算经验协方差矩阵，同时进行年化处理（乘以 252）

    stock_list = daily_stock_return.columns.tolist()
    ...
    demeaned_daily_stock_return = daily_stock_return - daily_stock_return.mean()
    ...
    for stock in stock_list:
        ...
        estimated_var.loc[stock] = volatility_exp_weight.dot(demeaned_daily_stock_return[stock].values * demeaned_daily_stock_return[stock].values) / volatility_exp_weight.sum()

    for lag in range(1, 6):

        demeaned_lag_stock_return = multiperiod_daily_stock_return['lag_' + str(lag)] - multiperiod_daily_stock_return['lag_' + str(lag)].mean()

        for stock in stock_list:

            exp_weighted_var.loc[stock] = correlation_exp_weight.dot(demeaned_lag_stock_return[stock].values * demeaned_lag_stock_return[stock].values) / correlation_exp_weight.sum()
