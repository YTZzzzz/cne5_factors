...
def get_daily_standard_deviation(stock_excess_return, market_cap_on_current_day):
    ...
    weighted_stock_standard_deviation = (stock_excess_return.replace(np.nan, 0) - stock_excess_return.mean()).pow(2).T.dot(exp_weight).pow(0.5)
...
def get_cumulative_range(stock_list, date, market_cap_on_current_day):
    ...
    daily_return = rqdatac.get_price(stock_list, trading_date_253_before, date, frequency='1d', fields='ClosingPx').fillna(method='ffill').pct_change()[1:]
...
def get_momentum(stock_list, date, market_cap_on_current_day):
    ...
    daily_return = rqdatac.get_price(stock_list, trading_date_505_before, trading_date_21_before, frequency='1d', fields='ClosingPx').fillna(method='ffill').pct_change()[1:]
...
def get_style_factors(date):
    ...
    style_factors = pd.concat([size_exposure, non_linear_size_exposure, market_portfolio_beta_exposure, residual_volatility_exposure, momentum_exposure, liquidity_exposure], axis = 1)
